...
const snapshot = (x: number, z: number, ack = -1): PlayerSnapshot => ({ id: "p", name: "Player", position: { x, y: 1.1, z }, velocity: { x: 0, y: 0, z: 0 }, yaw: 0, knockback: 0, score: 0, assists: 0, falls: 0, combo: 0, stocksRemaining: 3, eliminated: false, lastProcessedInput: ack, blocking: false, protected: false, ready: false, host: true });

describe("MovementPredictor", () => {
  it("moves immediately from local input", () => {
    const predictor = new MovementPredictor(); predictor.reconcile(snapshot(0, 0)); predictor.setEnabled(true);
  ...
  it("prunes acknowledged inputs and softly corrects small errors", () => {
    const predictor = new MovementPredictor(); predictor.reconcile(snapshot(0, 0)); predictor.setEnabled(true);
    predictor.recordInput(1); predictor.recordInput(2); predictor.reconcile(snapshot(1, 0, 1));
  ...
  it("snaps large divergence to the authority", () => {
    const predictor = new MovementPredictor(); predictor.reconcile(snapshot(0, 0));
  ...
  it("does not visually tunnel through arena walls", () => {
    const predictor = new MovementPredictor(); predictor.reconcile(snapshot(-7.7, 0)); predictor.setEnabled(true);
    predictor.update(.2, 0, -1, Math.PI / 2);
